...
const Len INFINITY__ = numeric_limits<Len>::max() / 4;
...
public:
    AStar(int n, Adj adj, Adj cost, std::vector<std::pair<Len,Len>> xy)
        : n_(n), adj_(adj), cost_(cost), distance_(2, vector<Len>(n_, INFINITY__)), visited_(n), xy_(xy)
    { workset_.reserve(n); }
    ...
    void clear() {
        for (int i = 0; i < workset_.size(); ++i) {
            int v = workset_[i];
            distance_[0][v] = distance_[1][v] = INFINITY__;
            visited_[v] = false;
        }
        workset_.clear();
    }
    ...
    Len query(int s, int t) {
		...
		while (!q.empty())
        {
			...
			if (v == t)
			{
				if(distance_[t] != INFINITY__)   return distance_[t];
				else        					return -1;
			}
			if (visited_[v] != true)
			{
