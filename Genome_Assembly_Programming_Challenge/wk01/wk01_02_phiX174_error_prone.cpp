...
std::string assemble_genome(std::vector<std::string> reads, const int mer) noexcept
{
    ...
    while(reads.size() > 1)
    {
        ...
        if(overlaps.size() > 3)
        {
            ...
            for(int i = 0, n = overlaps[overlaps.size()-4]; i < n; ++i, ++suffix, ++prefix1, ++prefix2, ++prefix3, ++prefix4)
            {
                if (*suffix == *prefix1 && *prefix1 == *prefix2 && *prefix2 == *prefix3 && *prefix3 == *prefix4)
                    continue;
...
std::vector<std::string> read_reads()
{
